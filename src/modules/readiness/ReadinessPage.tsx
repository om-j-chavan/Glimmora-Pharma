  ...
  return (
      ...
      <Modal open={addCardOpen} onClose={() => { setAddCardOpen(false); reset(); }} title="Add readiness action">
        <form onSubmit={handleSubmit(onCardSave)} noValidate className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><p className="text-[11px] font-medium mb-1" style={{ color: "var(--text-secondary)" }}>Owner *</p><Controller name="owner" control={control} render={({ field }) => <Dropdown options={users.map((u) => ({ value: u.id, label: `${u.name} (${roleLabel(u.role)})` }))} value={field.value} onChange={field.onChange} placeholder="Select owner" width="w-full" />} />{errors.owner && <p className="text-[11px] text-[#ef4444] mt-1">{errors.owner.message}</p>}</div>
