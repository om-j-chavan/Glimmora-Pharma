...
import { Paperclip, Send, Clock, CheckCircle2, FileText, X } from "lucide-react";
...
const ROLE_LABEL: Record<string, string> = {
  qa_head: "QA Head", super_admin: "Super Admin", customer_admin: "Administrator",
  regulatory_affairs: "Regulatory Affairs", csv_val_lead: "CSV/Val Lead",
  operations_head: "Operations Head", qc_lab_director: "QC Lab Director", it_cdo: "IT / CDO", viewer: "Viewer",
};
 ...
 *   • do — start, attach/remove their OWN task docs, submit with notes.
  ...
  return (
    ...
    >
      <div className="space-y-3">
        ...
        {/* Worker's own task documents — upload + remove */}
        <div>
          ...
          {task.taskDocs.length === 0 ? (
            <p className="text-[11px] italic" style={{ color: "var(--text-muted)" }}>No documents uploaded yet.</p>
          ) : (
            <ul className="space-y-1">
              {task.taskDocs.map((d) => (
                <DocRow key={d.id} doc={d} onRemove={canWork ? () => void removeDoc(d.id) : undefined} removing={busy} />
              ))}
            </ul>
          )}
          {canWork && (
            <label className="inline-flex items-center gap-1.5 text-[12px] cursor-pointer px-2.5 py-1.5 mt-1.5 rounded-lg border" style={{ borderColor: "var(--bg-border)", color: "var(--text-secondary)", opacity: busy ? 0.6 : 1 }}>
        ...
        return (
          <div key={m.id} className="rounded-lg p-2" style={{ background: isMine ? "var(--brand-muted)" : "var(--bg-surface)", border: "1px solid var(--bg-border)" }}>
            <div className="flex items-center gap-1.5 mb-0.5">
              ...
              <Badge variant={isQA ? "purple" : "gray"}>{isQA ? "QA" : (ROLE_LABEL[m.authorRole] ?? "Worker")}</Badge>
