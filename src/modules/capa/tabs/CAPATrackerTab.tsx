...
export function CAPATrackerTab({
  ...
  isDark, isViewOnly, users, user, timezone, dateFormat,
        ...
        <div className={clsx(selectedCAPA ? "lg:col-span-2" : "", "overflow-x-auto")}>
          {displayed.length === 0 ? (
            ...
            <table className="data-table" aria-label="CAPA register">
              ...
              <tbody>
                {displayed.map((c) => (
                  <tr key={c.id} onClick={() => onSelectCAPA(selectedCAPA?.id === c.id ? null : c)} className="cursor-pointer" aria-selected={selectedCAPA?.id === c.id}
          ...
          <aside aria-label="CAPA detail" className="card lg:col-span-1">
            <div className="card-header">
              ...
              <div className="flex items-center gap-1 ml-auto">
                ...
                <button type="button" onClick={() => onSelectCAPA(null)} aria-label="Close CAPA detail" className="w-5 h-5 flex items-center justify-center shrink-0 border-none bg-transparent outline-none cursor-pointer opacity-40 hover:opacity-100 transition-opacity">
