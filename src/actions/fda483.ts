...
export async function updateFDA483Event(
  ...
  const session = await requireAuth();
  // Validate the partial update at runtime (mirrors every sibling action in this
  // file). Previously the schema was only used for its inferred type and the raw
  // input was spread straight into the write — unvalidated fields could reach
  // Prisma. .partial() keeps the legacy partial-edit semantics.
  const parsed = UpdateEventSchema.partial().safeParse(input);
  if (!parsed.success) {
    return { success: false, error: "Validation failed", fieldErrors: parsed.error.flatten().fieldErrors };
  }
...
export async function closeObservation(
  ...
  const session = await requireAuth();
  // SME Pass 2 — closing an observation is a QA-authority quality action; the
  // admin identities (customer_admin view-only, super_admin platform-only) are excluded.
  if (session.user.role !== "qa_head") {
    return { success: false, error: "Only a QA Head can close an observation." };
  }
  const parsed = CloseObservationSchema.safeParse(input);
  if (!parsed.success) {
    return { success: false, error: "Validation failed", fieldErrors: parsed.error.flatten().fieldErrors };
  }
  // Only qa_head reaches here (a tenant user), so the lookup is always
  // tenant-scoped — the prior super_admin cross-tenant branch is gone.
  const existing = await prisma.fDA483Observation.findFirst({
