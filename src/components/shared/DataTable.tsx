...
/**
 * Canonical, config-driven data table for the whole app.
 *
 * Markup lives here once; callers describe columns + data and let render
 ...
 *
 * Styling comes from the global `.data-table` class (sticky header, zebra,
 * hover, density, print) defined in index.css. Sorting and pagination are
 * opt-in and off by default, so adopting this component never changes a
 * table's behaviour unless the caller asks for it.
 */
...
export function DataTable<T>({
  ...
  const table = (
    <table className={tableClass} aria-label={ariaLabel} style={tableStyle}>
      ...
      {!headerless && (
        <thead>
          <tr className={isFixed ? "border-b border-(--bg-border)" : undefined}>
            {visibleColumns.map((col) => {
        ...
        {showSkeleton ? (
          ...
          sortedData.map((row, i) => (
            <tr
              key={keyOf(row)}
              className={clsx(
                isFixed && "hover:bg-(--bg-surface) transition-colors",
                isFixed && i < sortedData.length - 1 && "border-b border-(--bg-border)",
            ...
            >
              {visibleColumns.map((col) => {
                ...
                return (
                  <td
                    key={col.key}
                    className={clsx(isFixed && "px-4 py-3", cellCls)}
                    style={style}
                  >
                    {col.render(row, i)}
                  </td>
        ...
        <tfoot>
          <tr className="border-t border-(--bg-border)">
            {visibleColumns.map((col) => (
              <td
                key={col.key}
                className={clsx(isFixed && "px-4 py-3", col.cellClassName)}
                style={col.align && col.align !== "left" ? { textAlign: col.align } : undefined}
              >
                {col.footer}
              </td>
